...
echo "waiting for bspwm_window_titles to die"
while pgrep -u $UID bspwm_window_titles.sh >/dev/null; do sleep 1; done
...
bspc subscribe node_focus node_remove desktop_focus | while read -r _; do
    ...
    for monitor in $monitors; do
        # index
        index=$((index + 1))
        export BSPWM_WINDOW_LIST="$HOME/.cache/bspwm_windows_${index}.txt"
        ...
        winids_on_desktop=$( bspc query -N -n .window.!hidden -m "$monitor" -d "$last_focused_desktop" )
