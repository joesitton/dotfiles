...
def append_output(title, action):
    ...
    if len(results) < 2:
        ...
        results.insert(99, create_result(title, action))
...
def google(query):
    sleep(0.4)  # so we aren't querying EVERYTHING we type
...
def find(query):
    ...
    for i in xrange(min(5, len(find_array))):
        append_output(str(find_array[i]), "urxvt -e bash -c 'if [[ $(file " + find_array[
                      i] + " | grep text) != \"\" ]]; then vim " + find_array[i] + "; else cd $(dirname " + find_array[i] + "); bash; fi;'")
    update_output()
